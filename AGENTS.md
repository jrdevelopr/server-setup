...
## Project overview
A reusable kit that turns a fresh Linux server into a self-hosting **app lab**: Docker **and**
...
```bash
./configure.sh                       # interactively writes lab.conf (the only values you set)
./bootstrap.sh                       # wires the host from lab.conf (timezone, git, secrets dir, caddy, units)
...
- **Don't ask for approval on routine work.** Installs, builds, deploys, file edits, git
  ...
  the human can give you — secrets, the interactive Cloudflare/GitHub logins, or `LAB_PASSWORD`.
...
- **NEVER commit or push secrets or env files.** No `.env`, `*.env`, `lab.conf`, passwords, API
...
- **Secrets live host-local only:** `lab.conf` (gitignored, `chmod 600`) and `/etc/<LAB_NAME>/*.env`
  (root, `chmod 600`).
...
- **One shared production password for everything** (the splash login *and* each app's own
  login): **`LAB_PASSWORD`** from `lab.conf`. The human sets it before you start — never invent
...
- **Username is always `admin`** — or an **email** where an app requires one, using
  **`ADMIN_EMAIL`** from `lab.conf` (`ADMIN_USER` holds the default).
...
- Timezone, git identity, ports, and all the above credentials come from **`lab.conf`** (set once
